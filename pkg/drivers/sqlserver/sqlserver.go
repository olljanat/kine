...
func New(ctx context.Context, dataSourceName string, tlsInfo tls.Config, connPoolConfig generic.ConnectionPoolConfig, metricsRegisterer prometheus.Registerer) (server.Backend, error) {
	...
	// dialect, err := generic.Open(ctx, "sqlserver", parsedDSN, connPoolConfig, "?", false, metricsRegisterer)
	if err != nil {
		return nil, err
	}
...
func setupGenericDriver(ctx context.Context, driverName, dataSourceName string, paramCharacter string, numbered bool) (*generic.Generic, error) {
	...
	for i := 0; i < 300; i++ {
		db, err = generic.OpenAndTest2(driverName, dataSourceName)
		if err == nil {
			break
		}
	...
	return &generic.Generic{
		DB: db,
		GetRevisionSQL: generic.QueryBuilder2(fmt.Sprintf(`
			SELECT
			0, 0, %s
			FROM kine kv
			WHERE kv.id = ?`, columns), paramCharacter, numbered),

		GetCurrentSQL:        generic.QueryBuilder2(fmt.Sprintf(listSQL, ""), paramCharacter, numbered),
		ListRevisionStartSQL: generic.QueryBuilder2(fmt.Sprintf(listSQL, "AND mkv.id <= ?"), paramCharacter, numbered),
		GetRevisionAfterSQL:  generic.QueryBuilder2(fmt.Sprintf(listSQL, idOfKey), paramCharacter, numbered),

		CountSQL: generic.QueryBuilder2(fmt.Sprintf(`
			SELECT (%s), COUNT(c.theid)
			FROM (
				%s
			) c`, revSQL, fmt.Sprintf(listSQL, "")), paramCharacter, numbered),

		AfterSQL: generic.QueryBuilder2(fmt.Sprintf(`
			SELECT (%s), (%s), %s
			FROM kine kv
			WHERE
				kv.name LIKE ? AND
		...
		DeleteSQL: generic.QueryBuilder2(`
			DELETE FROM kine
			WHERE id = ?`, paramCharacter, numbered),

		UpdateCompactSQL: generic.QueryBuilder2(`
			UPDATE kine
			SET prev_revision = ?
			WHERE name = 'compact_rev_key'`, paramCharacter, numbered),

		InsertLastInsertIDSQL: generic.QueryBuilder2(`INSERT INTO kine(name, created, deleted, create_revision, prev_revision, lease, value, old_value)
			values(?, ?, ?, ?, ?, ?, ?, ?);select SCOPE_IDENTITY()`, paramCharacter, numbered),

		InsertSQL: generic.QueryBuilder2(`INSERT INTO kine(name, created, deleted, create_revision, prev_revision, lease, value, old_value)
			values(?, ?, ?, ?, ?, ?, ?, ?); select SCOPE_IDENTITY()`, paramCharacter, numbered),

		FillSQL: generic.QueryBuilder2(`INSERT INTO kine(id, name, created, deleted, create_revision, prev_revision, lease, value, old_value)
			values(?, ?, ?, ?, ?, ?, ?, ?, ?);select SCOPE_IDENTITY()`, paramCharacter, numbered),
		RevSQL: revSQL,
